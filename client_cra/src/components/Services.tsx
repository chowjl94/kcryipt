...
const ServiceCard = ({ color, title, icon, subtitle }: ParameterProps) => (
  <div className="flex flex-col justify-start items-center white-glassmorphism p-3 m-2 hover:shadow-xl">
    <div
      className={`w-10 h-10 rounded-full flex flex-col justify-center items-center ${color}`}
    >
      {icon}
    </div>
    <div className="flex flex-col flex-1 items-center text-ellipsis">
      <h3 className="mt-2 text-center text-ellipsis text-white lg md:w-9/12">
        {title}
      </h3>
    </div>
    <div className="flex flex-col flex-1 items-center text-ellipsis">
