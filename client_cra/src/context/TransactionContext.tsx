...
export interface ITransasctionContext {
  connectWallet: ()=>(Promise<void>|string),
  account: ""|string,
  formData:IFormData,
  isTrxLoading:Boolean,
  tranxes: ITrxes[],
  handleChange: any
  // handleChange(e:React.ChangeEvent<HTMLInputElement>,name:string):(void),
  sendTransaction:()=>(Promise<void>|string),
  ethereum:ExternalProvider,
}

export const TransactionsContext = React.createContext<ITransasctionContext|undefined>(undefined);
``
...
export const TransactionsProvider = ({ children }: ChildProps) => {
  ...
  const getTransactions = async () => {
    try {
      ...
      console.log(trxes);
      const trxData = trxes.map((t: { receiver:string ; sender: any; timestamp: number; keyword: string; amount: string; }) => ({
  ...
  const sendTransaction = async () => {
    try {
      ...
      const transactionID = await transactionContract.addtoChain(
        addressTo,
        amtInGwei,
        messageTo,
        gif
